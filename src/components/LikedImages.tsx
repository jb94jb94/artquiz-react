...
export function LikedImages() {
  const { likedArtworks, toggleLike, loadingLikes: loadingLikes } = useUserData();
  ...
  useEffect(() => {
    ...
    likedArtworks.forEach((imageId) => {
      ...
      if (!artist) return;
      const src = `https://flourishing-sfogliatella-525af4.netlify.app/${artist.folder}/${numberStr}.jpg`;
